...
* **`configuracao_e_gerenciamento_com_ssh.pdf`**: Conjunto de slides utilizado na apresentação do seminário. O documento aborda o contexto histórico, o funcionamento do `ssh-agent`, regras de hardening e exemplos de *port forwarding* (Local, Remote e Dynamic).
...
* **`Dockerfile`**: Arquivo utilizado para construir o contêiner que atua como o servidor remoto da demonstração. Ele utiliza uma imagem do Ubuntu, instala o servidor SSH e injeta automaticamente a chave pública no usuário administrador.
